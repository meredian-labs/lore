...
See [`lore log`](./log.md) for full documentation of `glh log` output.
...
- [`lore log`](./log.md) — `lore log` and `glh log` output reference
- [`lore init`](./init.md) — sets `core.hooksPath` so Git hooks fire automatically
- [`lore record`](../cli/record.md) — emit a note task without committing
